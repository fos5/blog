...
import dev.festus.blog.exception.customExceptions.ApplicationException;
...
public class GlobalException {
    ...
    public ResponseEntity<?> conflictRequest(MethodArgumentNotValidException ex, WebRequest request){
        List<String> errors = ex.getBindingResult().getFieldErrors()
                ...
                .toList();
        ErrorDetails errorDetails = new ErrorDetails(new Date(), errors.get(0), request.getDescription(false), 400);
        return new ResponseEntity<>(errorDetails, HttpStatus.BAD_REQUEST);
    }
